...
import { WeeklyRhythmProgress, groupWeeklyRhythmTasks } from '@/lib/weekly-rhythm'
...
interface WeeklyPlannerGridProps {
  rhythms: Rhythm[]
  logs: RhythmLog[]
  progressByRhythmId?: Record<string, WeeklyRhythmProgress>
...
export function WeeklyPlannerGrid({
  ...
  const logMap = new Map<string, RhythmLog>(
    localLogs.map((log) => [`${log.rhythm_task_id}::${log.completed_at}`, log])
  )
  ...
  function handleToggle(taskId: string, date: string, note?: string) {
    ...
    if (alreadyDone) {
      setLocalLogs((prev) =>
        prev.filter(
          (log) => !(log.rhythm_task_id === taskId && log.completed_at === date)
        )
      )
    } else {
      setLocalLogs((prev) => [
  ...
  return (
    <div className="bg-white border border-border/50 rounded-3xl shadow-sm overflow-hidden">
            ...
            >
              ...
              <p
                className={cn(
                  ...
                  dayIsToday ? 'text-primary' : 'text-foreground/70'
                )}
              >
      ...
      {/* Rhythm Groups */}
      {rhythms.map((rhythm) => {
        const progress = progressByRhythmId[rhythm.id]

        return (
          <div key={rhythm.id} className="border-b border-border/30 last:border-b-0">
            {/* Group Label */}
            <div className="px-4 py-2 bg-slate-50/80 border-b border-border/20 text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {progress && (
                  <div className="rounded-full border border-border/50 bg-white px-2.5 py-1 text-[10px] font-semibold normal-case tracking-normal text-slate-600">
                    {progress.completed}/{progress.scheduled} done - {progress.completionRate}%
                  </div>
                )}
            ...
            {groupWeeklyRhythmTasks(rhythm.weekly_rhythm_tasks || []).map((task) => {
              ...
              const gridDayOfWeek = [1, 2, 3, 4, 5, 6, 0]
...
function GridCell({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-72 rounded-2xl shadow-xl p-4 border-border/50"
        align="center"
      >
