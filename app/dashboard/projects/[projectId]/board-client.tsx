...
export function BoardClient({ project, initialTasks, canEdit = true }: { project: any, initialTasks: any[], canEdit?: boolean }) {
          ...
          <BoardColumn key={status} status={status} config={config} count={colTasks.length}>
            {colTasks.length === 0 ? (
              ...
              <div className="space-y-3">
                {colTasks.map(task => (
                  <TaskCard key={task.id} task={task} projectId={project.id} canEdit={canEdit} />
                ))}
              </div>
            )}
          </BoardColumn>
